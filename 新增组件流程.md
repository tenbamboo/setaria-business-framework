...
6. 如需引入样式文件的话，需要按照以下步骤

- 需要在`packages\theme-chalk\src\`下新增对应`scss`文件
- 在`packages\theme-chalk\src\index.scss`文件中引入对应文件
- 还需在对应组件文件夹下的`style`中 2 个文件引入对应样式文件

### 本地调试组件

- 本地执行`pnpm run dev` 并可在`play\src\App.vue`文件下测试组件内容，此方式为独立调试方式(也可通过`文档调试组件`的方式调试)
